...
- After departure every route is reduced to its remaining travel (`Route.remaining_after_elapsed`, by typical segment durations since `started_at`), so ETAs hold steady as the clock advances. Fares and door-to-door minutes still use the full route from the departure time.
- Both rail routes share `walk-tampines`, `wait-ewl` and `ride-ewl-bugis`; only `ride-ewl-city` (Bugis to Raffles Place) is affected by the synthetic fault. The fault (46 to 54 minutes from 07:48) holds services at the affected stretch until it clears, so a route is delayed only by what is left of the fault when the commuter reaches that stretch.
- A route is feasible only until its decision deadline (bus 07:48, Bugis switch 08:06). Infeasible routes are returned with `available: false` and are excluded from fares and from the policy.
- Known gaps: progress is inferred from elapsed time, not a position fix; monitoring is driven by explicit evaluations and demo controls. A background worker was left out deliberately because the demo clock only moves when a control is pressed.
